...
from mongodb_models import DetectionTask
...
def predict_image_class(task_id, images_path, pre_process_path, model, res_dict):
    ...
    for idx, img_name in enumerate(pre_process_files):
        ...
        logger.info(f"Task_id:{task_id} Preprocessing FILE: {img_name} current progress: {idx}/{file_count} ")
        update_task_info(task_id, TaskInfoKey.LOG.value, f" Task_id:{task_id} Preprocessing FILE: {img_name} Current Progress {idx}/{file_count}")
        ...
        for index, obj in detected_objects.iterrows():
            ...
            update_task_info(task_id, TaskInfoKey.LOG.value, f'Task: {task_id} The file {img_name} predicted current object class is: {label_text} axis: {x1} {y1} {x2} {y2}')
...
def handler(detect_floder, task_id, node):
    ...
    update_task_info(task_id, TaskInfoKey.LOG.value, f"Preprocessing {idx}/{file_count}")
