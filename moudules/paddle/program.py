...
from log_handler import logger as custom_logger
...
class ArgsParser(ArgumentParser):
    def __init__(self):
        if os.path.exists("configs/en_PP-OCRv3_rec.yml"):
            config_file = "configs/en_PP-OCRv3_rec.yml"
        else:
            config_file = "moudules/paddle/configs/en_PP-OCRv3_rec.yml"

        super(ArgsParser, self).__init__(
            formatter_class=RawDescriptionHelpFormatter)
        self.add_argument("-c", "--config", default=config_file, help="configuration file to use")
        self.add_argument(
...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        logger.warning(f"{epoch}/{epoch_num}")
        ...
        total_count = len(train_dataloader)
        for idx, batch in enumerate(train_dataloader):
            ...
            if dist.get_rank() == 0 and (
                ...
                strs = 'Task ID: {} epoch: [{}/{}], Idx: {}/{} global_step: {}, {}, avg_reader_cost: ' \
                       '{:.5f} s, avg_batch_cost: {:.5f} s, avg_samples: {}, ' \
                       'ips: {:.5f} samples/s, eta: {}'.format(task_id,
                                                               epoch, epoch_num, idx, total_count, global_step, logs,
                                                               train_reader_cost / print_batch_step,
                                                               train_batch_cost / print_batch_step,
                                                               total_samples / print_batch_step,
                                                               total_samples / train_batch_cost, eta_sec_format)
                logger.warning(strs)
                update_task_info(task_id, TaskInfoKey.LOG.value, strs)
                # res = get_task_info(task_id)
                # logger.warning(res)
        ...
        try:
            pairs = [pair.strip().split(':') for pair in logs.split(',')]
            result_dict = {key.strip(): float(value) for key, value in pairs}
            result_dict.pop('lr')
            result_dict.pop('norm_edit_dis')
            train_info_list[f'epoch_{epoch}'].append(result_dict)
        except:
            print("not one")
